...
class EditReturnResults extends Component {
  constructor() {
    super();
    this.state = {
      flights: [],
  ...
  async handleSubmit (id){
    ...
    for(let i = 0; i < res.data.returnFlightSeats.length; i++){
      arr.splice(arr.indexOf(res.data.returnFlightSeats[i]),1)
    }
    axios
        .put('http://localhost:8082/api/Flights/'+res.data.returnFlightID, {SeatsBooked : arr})
        .then(res => {
        })
        .catch(err => {
    ...
    axios
        .put('http://localhost:8082/api/booking/'+this.props.match.params.id, {returnFlightID : id, returnFlightSeats: []})
        .then(res => {
...
export default EditReturnResults;